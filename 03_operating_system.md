...
extensions) lives in **`.env`** (build-cache output path derived in `lib/common.sh`), shared by all the step-03 scripts. A clean run builds and
...
3. Applies a control-plane patch to every node: the **VIP** bound to the wired NIC, `allowSchedulingOnControlPlanes:
   ...
   in `.env`), and the **Cilium prep** — `cluster.network.cni.name: none`,
